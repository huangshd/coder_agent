...
def prepare_chains(output_len: int):
    # Map phase uses high-throughput instance (GPU 0)
    map_llm = ChatOpenAI(
        temperature=0,
        model_name="gpt-3.5-turbo-map",  # Route to throughput-optimized worker
        max_tokens=output_len
    )

    # Reduce phase uses low-latency instance (GPU 1)
    reduce_llm = ChatOpenAI(
        temperature=0,
        model_name="gpt-3.5-turbo-latency",  # Route to latency-optimized worker
        max_tokens=output_len
    )
    ...
    map_chain = LLMChain(llm=map_llm, prompt=map_template)
...
async def send_request(
    ...
    # Map phase: runs on GPU 0 (high throughput)
    docs = await asyncio.gather(*coros)
    docs = "\n".join(docs[:4000]) # prevent stuck

    # Reduce phase: runs on GPU 1 (low latency)
